...
/**
 * W3C JWT-VC credential format handler.
 * Parses compact JWT-VC tokens where the credential is embedded in the JWT payload.
 */
export class W3cJwtVcFormat implements ICredentialFormat {
  ...
  async selectDisclose(raw: string, _claimPaths: string[]): Promise<string> {
    // W3C JWT-VC does not support selective disclosure natively
    return raw;
  }
  ...
  toDisplayModel(parsed: ParsedCredential): VerifiableCredential {
    ...
    return {
      ...
      expirationDate: parsed.expirationDate ?? new Date(Date.now() + 5 * 365 * 864e5).toISOString(),
